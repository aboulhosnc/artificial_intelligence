...
# n_type = 'normal3' # just power normalized
...
if(n_type == 'standard'):
	...
	alpha = 0.0001
# just time variable normalized
elif(n_type == 'normal'):
	errorThreshold = [53,35,2.5]
	alpha = 0.1
# just power normalized
elif(n_type == 'normal3'):
	errorThreshold = [0.00001,0.0000001,0.0000001]
	alpha = 0.000000011
#both time and power normalized
else:
	errorThreshold = [0.97,0.68,0.05]
	alpha = 0.01
...
def normalizationMethod(data,type ):
	if(type == 'standard'):
		...
		# print ('method both time and power are normalized')
		data['power'] = (data['power'] - data['power'].min()) / (data['power'].max() - data['power'].min())
		data['time'] = (data['time'] - data['time'].min()) / (data['time'].max() - data['time'].min())
		# return data
