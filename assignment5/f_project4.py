...
def formNewCluster(clusterWeights, pattern):
	return -1
...
def formingClustersAsNeeded(vectorMatrix):
	...
	for c in clusterWeights:
		print(c)
	print(clusterPattern)

	pn = Counter(clusterPattern).keys()
	pc = Counter(clusterPattern).values()
...
formingClustersAsNeeded(termDocMatrix)
