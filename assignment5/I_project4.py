...
#counts the total frequency of each word
...
#extracts the top 10 most frequent words
count = 0
maxNumber = 10
for w in wordFreq.columns.values.tolist():
	mostFreq.append(w)
...
termDocMatrix.rename(index = { i : 'sentence {}'.format(i+1) for i in range(len(termDocMatrix.index))}, inplace = True)

clusterMatrix = formingClustersAsNeeded(termDocMatrix)

clusterMatrix.rename(index = { i : 'cluster {}'.format(i+1) for i in range(len(clusterMatrix.index))}, inplace = True)

print('Feature List:', mostFreq)
